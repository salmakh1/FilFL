...
class trainClientsFL(object):
    ...
    def train_clients(self):
        ...
        train_sets, val_sets, public_data, data_weights = datamodule.load_and_split(self.cfg)
        logging.info(f"data_weights is {data_weights}")
        ...
        for client in clients:
            # schedulers.append(torch.optim.lr_scheduler.StepLR(client.optimizer, step_size=20, gamma=0.988))
            schedulers.append(torch.optim.lr_scheduler.MultiStepLR(client.optimizer, milestones=[30,60,90,120,160], gamma=0.1)) #[30,90,150]
            # schedulers.append(torch.optim.lr_scheduler.CosineAnnealingLR(
            #     client.optimizer, len(client.train_set) * self.cfg.rounds, eta_min=1e-3))
        ...
        for t in range(self.cfg.rounds):
            ...
            if cond:  # used for divfl
                ...
                if self.cfg.bandit:
                    ...
                    weights_of_active_clients = {client_idx: weight for client_idx, weight in data_weights.items() if client_idx in active_clients_indices}
            ...
            if self.cfg.bandit and t % self.cfg.periodicity_of_bandit == 0:
            ...
            logging.info(f"active clients {len(active_clients)}")
                ...
                if b or t % self.cfg.periodicity_of_bandit == 0:
                    ...
                    if self.cfg.client_filtering:
                        x, bandit_reward, random_reward, ps = optimized_rgl(model=client_object[0].model,
                                                                            ...
                                                                            device=self.device, data_set=client_object[0].train_set,
                                                                            p_bandit=1,
